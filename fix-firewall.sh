...
ufw allow 914:917/tcp comment 'OpenVPN du phong'   > /dev/null
ufw allow 914:917/udp comment 'OpenVPN du phong'   > /dev/null
...
if grep -q "OPENVPN NAT" "$UFW_BEFORE" 2>/dev/null; then
  ...
  #
  #   Nếu muốn client VPN ra ngoài bằng chính ip lấy được thì không cần NAT
